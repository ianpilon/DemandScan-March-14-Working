...
const AnalysisResults = ({ showResult, localAnalysisResults, longContextResults, gainAnalysis, jtbdAnalysis, apiKey }) => {
   ...
   * - Average > 4: Critical
   */
  // Format CURSE scores for display, handling both numeric and range formats
  const formatCurseScore = (score) => {
    // Handle different formats (number or range)
    if (typeof score === 'number') {
      return score.toFixed(1);
    } else if (typeof score === 'string' && score.includes('-')) {
      // For range format (e.g., '3-4')
      return score;
    } else {
      return '0';
    }
  ...
  const calculateSeverityFromCurseScore = (curseScore) => {
    ...
    // Handle new format with ranges and confidence levels
    if (typeof curseScore === 'object' && curseScore !== null) {
      // Check if curseScore contains range properties (new format)
      const hasRanges = Object.values(curseScore).some(value => 
        typeof value === 'object' && value !== null && 'range' in value
      );
      
      if (hasRanges) {
        // New format: Calculate average using the midpoint of each range
        let sum = 0;
        let count = 0;
        
        for (const key of ['Crucial', 'Ubiquitous', 'Recurring', 'Specific', 'Extreme']) {
          if (curseScore[key] && Array.isArray(curseScore[key].range) && curseScore[key].range.length === 2) {
            // Use midpoint of range as the score value
            const midpoint = (curseScore[key].range[0] + curseScore[key].range[1]) / 2;
            sum += midpoint;
            count++;
          }
        }
    ...
    // Original format: Calculate average of direct CURSE scores
    const scores = Object.values(curseScore);
    if (!scores.length) return 'Medium';
    
    // Filter out any non-numeric values (handles both old and new formats gracefully)
    const numericScores = scores.filter(score => typeof score === 'number');
    if (numericScores.length === 0) return 'Medium';
  ...
  const getSeverityBadgeStyles = (severity) => {
    // Use custom variant to use consistent styling across the app
    const variant = "custom";
    
    // Updated colors based on the new color scheme
    const className = 
      (severity === 'Critical' || severity === 5) ? "bg-[#F00D0D] text-white hover:bg-[#D00C0C]" :
      (severity === 'High' || severity === 4) ? "bg-[#FE8C8C] text-black hover:bg-[#FE7A7A]" :
      (severity === 'Medium' || severity === 'Moderate' || severity === 3) ? "bg-[#FFEE8C] text-black hover:bg-[#FFEB7A]" :
      (severity === 'Low' || severity === 2) ? "bg-[#D5EAFF] text-blue-800 hover:bg-[#C4E2FF]" :
      (severity === 'Minimal' || severity === 1) ? "bg-[#F0F7FF] text-blue-800 hover:bg-[#E0F0FF]" :
      "";
  ...
  const renderResults = () => {
    switch (showResult) {
      ...
      case 'frictionAnalysis':
        // Pass the entire frictionAnalysis object to ensure we get primaryGoal and inferences
